document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderCartOnPage() {
    ...
    for (const productId in cart) {
      ...
      total += subtotal;
      html += `<tr style="border-bottom: 1px solid #eee;"><td style="padding: 15px 8px;">${
        item.name
      }</td><td style="padding: 8px;"><input type="number" class="input-qty" value="${
        item.quantity
      }" min="1" data-id="${productId}" style="width: 60px; text-align: center;"></td><td style="text-align: right; padding: 8px;">Rp${subtotal.toLocaleString(
        "id-ID"
      )}</td><td style="text-align: center; padding: 8px;"><button class="tombol-hapus" data-id="${productId}" style="color: red; background: none; border: none; cursor: pointer;">Hapus</button></td></tr>`;
      pesanWA += `- ${item.name} (Jumlah: ${item.quantity})\n`;
    }
    html += "</tbody></table>";
    pesanWA += `\n*Total Belanja: Rp${total.toLocaleString("id-ID")}*`;
    cartContentEl.innerHTML = html;
    cartTotalEl.innerHTML = `<h4 style="text-align: right;">Total Belanja: Rp${total.toLocaleString(
      "id-ID"
    )}</h4>`;
    if (whatsappButtonEl) {
      whatsappButtonEl.href = `https://api.whatsapp.com/send?phone=${nomorWhatsapp}&text=${encodeURIComponent(
        pesanWA
      )}`;
  ...
  function updateHeaderCart() {
    ...
    setTimeout(function () {
      if (miniCartContentEl) {
        if (totalItems === 0) {
          miniCartContentEl.innerHTML =
            '<p class="woocommerce-mini-cart__empty-message" style="padding: 1em;">Tidak ada produk di keranjang.</p>';
        } else {
          let html =
            ...
            html += `
                            <li class="woocommerce-mini-cart-item">
                                <a>
                                    ...
                                    <span class="quantity">${
                                      item.quantity
                                    } × <span class="woocommerce-Price-amount amount">Rp${item.price.toLocaleString(
              "id-ID"
            )}</span></span>
                                </a>
                            </li>
          ...
          html += "</ul>";
          html += `<p class="woocommerce-mini-cart__total total"><strong>Subtotal:</strong> <span class="woocommerce-Price-amount amount">Rp${total.toLocaleString(
            "id-ID"
          )}</span></p>`;
          ...
          if (
            ...
            currentPath.includes(
              "/di-balik-setiap-butir-telur-cerita-keadilan-dari-pedesaan-malang/"
            ) ||
            currentPath.includes(
              "/bukan-sekadar-warna-ini-4-perbedaan-mendasar-telur-ayam-kampung-dan-negeri/"
            )
          ) {
  ...
  document.querySelectorAll(".tombol-tambah-produk").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      if (
        quantityWrapper &&
        quantityWrapper.classList.contains("quantity-input")
      ) {
...
window.onload = function () {
  ...
  // Tunggu sekitar 1.5 detik (animasi huruf selesai) sebelum memulai transisi
  setTimeout(function () {
    // Tambahkan class 'loaded' untuk memulai animasi geser ke bawah
    preloader.classList.add("loaded");

    // Sembunyikan elemen preloader sepenuhnya setelah animasi geser selesai
    setTimeout(function () {
      preloader.style.display = "none";
    }, 600); // Durasi ini harus sama dengan durasi transisi di CSS (#preloader.loaded)
  }, 1500); // Jeda awal sebelum preloader menghilang
};
